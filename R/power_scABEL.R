                         ...
                         nsims=1E5, details=FALSE, setseed=TRUE)
{
  ...
  if (design=="2x3x3") {
    ...
    # how can we incorporate s2D? can we?
    cvec <- c(1,2)
  }
  if (design=="2x2x4") {
    seqs <- 2
  ...
  if (details) {
    ...
    cat("p(BE-ABE)=", p["BEabe"],"; p(BE-wABEL)=", p["BEwl"],
        "; p(BE-PE)=", p["BEpe"],"\n")
    #if(p["s2wT_lt_0"]>0) cat("p(s2wT<0)=", p["s2wT_lt_0"],"\n")
    cat("\n")
  }
  # return the 'power'
  as.numeric(p["BE"])
...
# working horse
.power.scABEL <- function(mlog, sdm, C2, Emse, cvec, df, s2wR, dfRR, s2wT,
                          ...
                          ln_lBEL=log(0.8), ln_uBEL=log(1.25), alpha=0.05)
{
  ...
  for (iter in 1:chunks){
    ...
    # cap if CVcap not Inf
    if (is.finite(CVcap)){
      uABEL[s2wRs>s2cap] <- +sdcap*r_const
      lABEL[s2wRs>s2cap] <- -sdcap*r_const
    }
    #--- 90% CIs for T-R
