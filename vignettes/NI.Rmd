---
title: "PowerTOST &bull; Non-Inferiority"
lang: "en"
output:
  rmarkdown::html_vignette:
    ...
    toc_depth: 1
vignette: >
  %\VignetteIndexEntry{PowerTOST &bull; Non-Inferiority}
...
Arguments `targetpower`, `margin`, `theta0`, and `CV` have to be given as fractions, not percent.\
...
With `sampleN.noninf(..., details = FALSE, print = FALSE)` results are provided as a [data.frame](https://stat.ethz.ch/R-manual/R-devel/library/base/html/data.frame.html) with eight columns `Design`, `alpha`,  `CV`, `theta0`, `Margin`, `Sample size`, `Achieved power`, and `Target power`. To access *e.g.*, the sample size use either `sampleN.noninf[1, 6]` or `sampleN.noninf[["Sample size"]]`. We suggest to use the latter in your code for clarity.
...
power.noninf(design = "2x2x4", margin = 1.25, CV = 0.20,
             theta0 = 1.05, n = 32)
```
Therefore, that gives us some ‘headroom’ for *C~max~*.
```{r}
