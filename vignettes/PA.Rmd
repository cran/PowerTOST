...
With `x <- pa.ABE(...)`, `x <- pa.scABE(...)`, and `x <- pa.NTIDFA(...)` results are given as an S3 object<small> </small>^[Wickham H. *Advanced R.* 2019-08-08. [The S3 object system](http://adv-r.had.co.nz/S3.html).] which can be printed, plotted, or both.
...
`regulator` can be `"EMA"`, `"HC"`, or `"FDA"`.
...
If no additional arguments are passed, the defaults of `power.NTIDFDA()` are applied, namely `alpha = 0.05`, `theta1 = 0.80`, `theta2 = 1.25`. Note the default $\small{\theta_0}$ 0.975 for <span title="Narrow Therapeutic Index Drugs">NTIDs</span> since the FDA requires tighter batch release limits of ±5\% for them.
